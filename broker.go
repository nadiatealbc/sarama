...
const (
	// SASLTypeCRAM represents the SASL/CRAM mechanism (Kafka 2.0.0+)
	SASLTypeCRAM = "CRAM"
...
func (b *Broker) Open(conf *Config) error {
	...
	go withRecover(func() {
		...
		if conf.Net.SASL.Enable {
			...
			case SASLTypeCRAM:
				b.connErr = b.sendAndReceiveSASLCRAM()
...
func (b *Broker) sendAndReceiveSASLCRAM() error {
	...
	clientSHA1, err := scram.SHA1.NewClient(b.conf.Net.SASL.User, b.conf.Net.SASL.Password, b.conf.Net.SASL.AuthorizationID)
	if err != nil {
		Logger.Printf("Failed to create the sha client while authenticating with SASL to broker %s: %s\n", b.addr, err.Error())
		return err
	}
	...
	bytesWritten, err := b.conn.Write([]byte(firstMsg))
	b.updateOutgoingCommunicationMetrics(bytesWritten)
	if err != nil {
		Logger.Printf("Failed to write SASL auth header to broker %s: %s\n", b.addr, err.Error())
		return err
